...
# 1. Configuration
CONFIG = {
    "project_name": "Llama2-SST2-Standard-HF-MI",
    "run_name": f"llama2-7b-sst2-manual-qlora-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
    "model_name": "meta-llama/Llama-2-7b-hf",
    "dataset_name": "stanfordnlp/sst2",
    "output_dir": "<MODEL_STORAGE>/fine-tuning-project/old_fine_tuned_model/",
    "max_length": 256,
    "batch_size": 16,
    "learning_rate": 2e-4,
    "num_epochs": 2,
...
class SST2LlamaDataset(Dataset):
    def __init__(self, split, tokenizer, max_length=256, num_samples=None):
...
def train_epoch(model, dataloader, optimizer, device):
    """Training loop for one epoch using whole-sequence loss."""
    model.train()
    total_loss = 0
...
def evaluate(model, dataloader, device):
    """Evaluation loop using whole-sequence loss."""
    model.eval()
    total_loss = 0
...
def main():
    ...
    train_dataset = SST2LlamaDataset(
        "train", tokenizer, CONFIG["max_length"], num_samples=CONFIG["train_sample_limit"]
    )
    eval_dataset = SST2LlamaDataset(
        "validation", tokenizer, CONFIG["max_length"]
    )
