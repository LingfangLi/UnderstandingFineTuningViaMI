...
class UserConfig:
    ...
    # Single run targets
    TARGET_MODEL = "llama3" # , "gpt2" #["gpt2","llama2","llama3","qwen2"]
    TARGET_TASK =  "qa_coqa"

    # Path configuration
    MODEL_ROOT_DIR = r"/mnt/data1/users/sglli24/fine-tuning-project-1/old_version_finetuned_models/"
    OUTPUT_DIR = r"/users/sglli24/UnderstandingFineTuningViaMI/experiments/attention_matrix_analysis/old_attention_analysis_results/"

    # Fine-tuned model folder mapping
    FT_MODEL_MAP = {
        "gpt2": {
            "sentiment_yelp": "gpt2-yelp.pt",
            "sentiment_sst2": "gpt2-sst2.pt",
        ...
        "llama3": {
            ...
            "sentiment_sst2": "llama3.2-sst2.pt",
        ...
        "qwen2": {
            "sentiment_yelp": "Qwen2-0.5B_yelp_best.pt",
            "sentiment_sst2": "Qwen2-0.5B_sst2_best.pt",
            "qa_squad":       "Qwen2-0.5B_squad_best.pt",
            "qa_coqa":        "Qwen2-0.5B_coqa_best.pt",
            "mt_kde4":        "Qwen2-0.5B_kde4_best.pt",
            "mt_tatoeba":     "Qwen2-0.5B_tatoeba_best.pt",
        }
    }
    # Analysis parameters
    NUM_SAMPLES = 50
...
class SysConfig:
    ...
    TASKS = {
        ...
        "mt_kde4": {
            ...
            "formatters": {"llama2": PromptFormatter.llama2_kde4,"qwen2": PromptFormatter.simple_mt,
        ...
        "mt_tatoeba": {
            ...
            "processor": lambda d: [{"en": s["translation"]["en"]} for s in d],
            "formatters": {"llama2": PromptFormatter.llama2_tatoeba, "qwen2": PromptFormatter.simple_mt,
                           "gpt2": PromptFormatter.simple_mt, "llama3": PromptFormatter.simple_mt}
        }
    }
