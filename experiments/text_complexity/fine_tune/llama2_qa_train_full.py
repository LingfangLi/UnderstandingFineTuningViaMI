...
SQuAD. The active subset (`simple` or `complex`) is selected via the
EXPERIMENT_TYPE environment variable; default is `simple`.

Counterpart of: experiments/text_complexity/fine_tune/llama2_qa_train.py
(which is the original LoRA version).

Hyperparameters mirror src/Fine_tune/Question_answering/LlaMA2-7b-squad-full.py.
"""
import os
import json
...
config = {
    ...
    "train_index_path": f"{PROJECT_ROOT}/experiments/text_complexity/matrix_analysis/squad_lexically_{EXPERIMENT_TYPE}_subset_indices.txt",
    ...
    # Training params (mirror LlaMA2-7b-squad-full.py)
    "max_seq_length": 1024,
    "learning_rate": 2e-5,
    "batch_size": 4,
...
run_name = f"llama2-7b-squad-{EXPERIMENT_TYPE}-full-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
output_dir = os.path.join(OUTPUT_BASE_DIR, run_name)
os.environ.setdefault("WANDB_PROJECT", "MI_llama2-squad-text_complexity-full")


def prepare_datasets():
    print(f"Loading {config['dataset_name']}...")
    full_train = load_dataset(config['dataset_name'], split="train")
    ...
    train_dataset = full_train.select(train_idx)
    eval_dataset = full_train.select(test_idx)
...
def formatting_prompts_func(examples):
    ...
    if isinstance(examples['context'], list):
        return [format_single(c, q, a) for c, q, a in zip(examples['context'], examples['question'], examples['answers'])]
...
def main():
    ...
    print(f"Starting Llama-2-7B squad-{EXPERIMENT_TYPE} full FT: {run_name}")
    trainer.train()
